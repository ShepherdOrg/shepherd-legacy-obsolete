...
then
	echo "pulling latest shepherd version icelandair/shepherd:${LABS_SHEPHERD_VERSION}"
...
function upgrade(){
	...
	docker run \
	    ...
	    --rm \
		icelandair/shepherd:${LABS_SHEPHERD_VERSION} /code/deploy-images/upgrade_image_version.js ${IMAGEYAMLFILE}
}
